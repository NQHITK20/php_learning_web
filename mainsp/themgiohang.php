<?php
include("../admincp/config/config.php");//bug trong trang chưa thấy
session_start();
//them so luong
if(isset( $_GET['cong'])){
    $id=$_GET['cong'];
    ...
    header("Location:../../web/index.php?quanly=giohang");
}
if(isset( $_GET['tru'])){
    $id=$_GET['tru'];
    foreach($_SESSION['cart'] as $cart_item){
        ...
        if(isset($_SESSION['cart'])){
            ...
            foreach($_SESSION['cart'] as $cart_item){
                if($cart_item['id']==$id){
                    ...
                    'giasp'=>$cart_item['giasp'],'hinhanh'=>$cart_item['hinhanh'],'masp'=>$cart_item['masp']);//kiểu 2
                }
                else{
                    $product[]=array('tensanpham'=>$cart_item['tensanpham'],'id'=>$cart_item['id'],'soluong'=>$cart_item['soluong']
                    ,'giasp'=>$cart_item['giasp'],'hinhanh'=>$cart_item['hinhanh'],'masp'=>$cart_item['masp']);//kiểu 2
